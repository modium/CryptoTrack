...
class CryptoTableViewCellViewModel {
    let name: String
    let symbol: String
    let price: String // need to convert from Float
    let iconUrl: URL?
    var iconData: Data?
...
class CryptoTableViewCell: UITableViewCell {
    ...
    // Prepares a cell for reuse by setting itself to nil
    ...
    func configure(with viewModel: CryptoTableViewCellViewModel) {
        ...
        if let data = viewModel.iconData {
            iconImageView.image = UIImage(data: data) // restore cached image
        } else if let url = viewModel.iconUrl {
            let task = URLSession.shared.dataTask(with: url) { [weak self] data, _, _ in
                if let data = data {
                    viewModel.iconData = data // persists data
